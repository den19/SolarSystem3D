...
/// <summary>
/// Builds an unpacked Mars3Prefab under Resources with baked URP materials.
/// Menu: Solar System / Build Mars 3 Prefab
/// Batch: -executeMethod Mars3PrefabBuilder.BuildAndVerify
/// </summary>
public static class Mars3PrefabBuilder
{
    const string ModelAssetPath = "Assets/Resources/ProbeMeshes/Mars3/Mars3.obj";
    const string PrefabAssetPath = "Assets/Resources/ProbeMeshes/Mars3/Mars3Prefab.prefab";
    const string MaterialsFolder = "Assets/Resources/ProbeMeshes/Mars3/Materials";
    const string AlbedoFolder = "Assets/Resources/ProbeMeshes/Mars3";
    const string PrefabResourcePath = "ProbeMeshes/Mars3/Mars3Prefab";
    const string ModelResourcePath = "ProbeMeshes/Mars3/Mars3";
    /// <summary>HGA tip in mesh-local space (from Tools/_mars3_import mesh_info.json).</summary>
    static readonly Vector3 AntennaLocal = new Vector3(0.01f, 0.699f, -0.097f);

    [MenuItem("Solar System/Build Mars 3 Prefab")]
    public static void BuildAndVerify()
    {
        if (!File.Exists(ModelAssetPath))
        {
            Debug.LogError($"Mars3PrefabBuilder: missing model at {ModelAssetPath}");
        ...
        if (modelRoot == null)
        {
            Debug.LogError($"Mars3PrefabBuilder: failed to load GameObject from {ModelAssetPath}");
        ...
        if (bodyShader == null)
        {
            Debug.LogError("Mars3PrefabBuilder: no suitable body shader found.");
            EditorApplication.Exit(4);
            return;
        }

        var root = new GameObject("Mars3Prefab");
        try
        {
            ...
            {
                Debug.LogError("Mars3PrefabBuilder: unpacked mesh has no MeshFilters with sharedMesh.");
            ...
            Debug.Log($"Mars3PrefabBuilder: saved flat prefab with {meshFilterCount} mesh filters.");
        }
        finally
        {
        ...
        Debug.Log(
            $"Mars3PrefabBuilder: prefab={PrefabAssetPath} " +
        ...
        {
            Debug.LogError("Mars3PrefabBuilder: Resources load failed or prefab has no meshes.");
